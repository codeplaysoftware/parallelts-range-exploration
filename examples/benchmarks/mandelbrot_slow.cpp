...
int main(int argc, char* argv[]) {
  ...
  std::vector<int, aligned_allocator<pixel, 4096>> indices(vsize * base_size);
  ...
  for (auto i = 0; i < iterations; ++i) {
    auto start = std::chrono::system_clock::now();
    {
      ...
      auto gpu_indices = std::experimental::copy(exec, indices);

      auto indices_tmp = ranges::view::iota(0)
                       | ranges::view::take(vsize * base_size);
      std::experimental::transform(exec, indices_tmp, gpu_indices, Identity{});

      std::experimental::transform(exec, gpu_indices, gpu_image,
